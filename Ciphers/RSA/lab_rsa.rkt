...
;;takes a list of plaintext and converts each character to ascii
(define (PtoA PList)
  (cond
    ((null? PList) '())
    (else (cons (char->integer (car PList)) (PtoA (cdr PList))) ) 
  )
  )
;;converts ascii list to cipherlist
(define (Cipher Alist key)
  (cond
    ((null? Alist) '())
    (else (cons (mod-exp (car Alist) (cadr key) (car key)) (Cipher (cdr Alist) key)))
    )
  )

(define (encrypt plaintext key)
  (define PList (string->list plaintext))
  (define Alist (PtoA PList))
  (Cipher Alist key)
  )
...
;;takes a list of naturals "ciphertext" and returns a list representing ascii plaintext
(define (deCipher ciphertext m d)
  (cond
   ((null? ciphertext) '())
   (else (cons (mod-exp (car ciphertext) d m) (deCipher (cdr ciphertext) m d)))
   )
  )
...
(define (decrypt ciphertext key)
  (define m (car key))
  (define d (cadr key))
  (define Alist (deCipher ciphertext m d))
  (define PList (map integer->char Alist))
  (list->string PList)
  )
